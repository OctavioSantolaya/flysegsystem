...
use App\Filament\Resources\FormResponseResource\Pages\ListFormResponses;
use App\Filament\Resources\FormResponseResource\Pages\CreateFormResponse;
use App\Filament\Resources\FormResponseResource\Pages\ViewFormResponse;
use App\Filament\Resources\FormResponseResource\Pages\EditFormResponse;
        ...
        return $form
            ->schema([
                Wizard::make([
                    ...
                    Wizard\Step::make('Gestor')
                        ->schema([
                            Textarea::make('assigned_transport_info')
                                ->label('Información de Transporte Asignado')
                                ->rows(3)
                                ->disabled(fn($get) => !$get('needs_transport')),
                            Textarea::make('assigned_accommodation_info')
                                ->label('Información de Alojamiento Asignado')
                                ->rows(3)
        ...
        return $table
            ->columns([
                ...
                TextColumn::make('children_count')
                    ->label('Niños')
                    ->getStateUsing(fn ($record) => $record->children_count)
                    ->badge()
                    ->color('warning'),
                TextColumn::make('children_ages')
                    ->label('Edades')
                    ->getStateUsing(fn ($record) => $record->children_ages)
                    ->formatStateUsing(fn ($state) => $state && count($state) > 0 ? implode(', ', $state) . ' años' : 'N/A')
